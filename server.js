...
const Prestador = require('./models/prestador.model');
...
Veterinario.hasMany(Agendamento, { foreignKey: 'veterinarioId', onDelete: 'CASCADE' });
Agendamento.belongsTo(Veterinario, { foreignKey: 'veterinarioId' });

Servico.hasMany(Agendamento, { foreignKey: 'servicoId', onDelete: 'SET NULL' });
...
app.post('/agendamentos/editar/:id', async (req, res) => {
    await Agendamento.update(req.body, { where: { id: req.params.id } }); res.redirect('/agendamentos');
});
